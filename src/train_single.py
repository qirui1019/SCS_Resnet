...
# -----------------------训练过程--------------------------#
def train(batch_size, new_learning_rate, my_model, lr_decay, decay_rate, num_epochs):
    # 定义数据增强
    ...
    # 创建数据集对象
    train_set = FashionDataset('train', root_path, file_name1, transform=transform)
    ...
    test_set = FashionDataset('test', root_path, file_name2, transform=transform)
    ...
    optimizer = torch.optim.Adam(my_model.parameters(), lr=new_learning_rate, weight_decay=1e-4)
    # 添加学习率调度器
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        writer.add_scalar("Time/Train/single", epoch_time, epoch)

        # 计算所有epoch的平均精度和损失并将结果保存到列表中
        epoch_accuracy= total_hits / total_samples
        epoch_loss = total_loss / total_samples

        writer.add_scalar("Loss/train/single", epoch_loss, epoch)
        writer.add_scalar("Accuracy/train/single",  epoch_accuracy, epoch)
        ...
        writer.add_scalar("Loss/test/single", test_loss, epoch)

        writer.add_scalar("Accuracy/test/single", test_accuracy, epoch)

        model_path = os.path.join(model_save_path,
                                  f'lr_{new_learning_rate}_lr_decay_{lr_decay}_decay_rate{decay_rate}_bsize{batch_size}_num_epochs{num_epochs}.pth')
...
if __name__ == '__main__':
    ...
    decay_rates = [0.5]
    num_epochs = [30]
    param_combinations1 = itertools.product(batch_sizes, learning_rates, lr_decays, decay_rates, num_epochs)
    for batch_size, lr, lr_decay, decay_rate, num_epoch in param_combinations1:
        print(
        ...
        if cuda:
            model = model.cuda()
        train(batch_size, lr, model, lr_decay, decay_rate, num_epoch)
